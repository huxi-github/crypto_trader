...
def do_MA_condition_Analysis(data):
    #是否在30 均线上方
    if  float(data['Close'].iloc[-7]) > float(data['MA30'].iloc[-7])\
    and float(data['Close'].iloc[-6]) > float(data['MA30'].iloc[-6])\
    ...
    and float(data['MA30'].iloc[-7])  > float(data['MA99'].iloc[-7]):
        return True
    else:
        return False

def do_cacu_MA_last5(sysbol_pair:str,frame_level:str):
    data=get_symbol_data_of_last_frame_s(sysbol_pair,frame_level,'300')
...
def do_the_select_and_decision_fast():
    ...
    for (key, val) in top_symbols.items():
        ...
        if coin_pair in Last_Entry_TICKDate and Last_Entry_TICKDate[coin_pair] == pd.to_datetime(data['Date'].iloc[-1]/1000,unit='s'):
            ...
            print("同一根K线重复进入")
        if five_UP and do_MA_condition_Analysis(data) and not One_KLine_Same_Entry and do_increase_rate_Analysis(data): 
...
def do_5_continous_up_Analysis(data):
    #前K6->K5是跌的 K5->K1 是涨的
    if  data['Close'].iloc[-7] < data['Close'].iloc[-6]\
    and data['Close'].iloc[-6] < data['Close'].iloc[-5]\
...
def do_increase_rate_Analysis(data):
    print("do_increase_rate_Analysis")
    close = data['Close']
    # print(close)
    grow_24H =(float(close.iloc[-2])-float(close.iloc[24*2]))/float(close.iloc[24*2])
    grow_30m_1 =(float(close.iloc[-2])-float(close.iloc[-3]))/float(close.iloc[-3])
    draw_4days_avg = ((float(close.iloc[-2])-float(close.iloc[24*2*5]))/float(close.iloc[24*2*4]))/5
    print('grow_24H==%.4f'%(grow_24H)) 
    print('draw_4days_avg==%.4f'%(draw_4days_avg)) 
    if  grow_24H < 0.3\
    and grow_30m_1 < 0.08\
    and draw_4days_avg>-0.005:
        return True
    else:
        print("最近一段时间涨幅不符合条件")
        log_to_file("最近一段时间涨幅不符合条件",log_to_file_path)
        return False
