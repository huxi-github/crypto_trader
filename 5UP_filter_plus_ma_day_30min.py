...
SL_per =11
Frame_level= '30m'
log_to_file_path = "5UP_filter"+Frame_level+"_plus_ma.log"
golobal_data ="db_file/json/golobal_data_"+Frame_level+"_plus_ma"
...
DealMgr = DEALMGR('db_file/sqlite/trade_list_30m_sqlite3_plus_ma.db')
...
def do_MA_condition_Analysis(data):
    ...
    and float(data['MA99'].iloc[-1]) > float(data['MA20day'].iloc[-1]):
        return True
    else:
        return False

def do_cacu_MA_last5(sysbol_pair:str,frame_level:str):
    # 2*24*20==960 20day 均线
    data=get_symbol_data_of_last_frame_s(sysbol_pair,frame_level,'967')
    ...
    MA20day= data['Close'][-(960+6):].rolling(960).mean()
    data['MA7'] =  MA7_s
    data['MA30'] = MA30_s
    data['MA99'] = MA99_s
    data['MA20day'] = MA20day
    return data
