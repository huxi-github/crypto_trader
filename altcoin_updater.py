...
# 更新机器人设置
def start_A_smart_trade_with_on_line_pair(coin_pair:str=''):
    response1 = p3c.request(
        entity='smart_trades',
        action='create_smart_trade',
        payload={
                 "name" :"on_coin_line_profit_trade",
                 "pairs":coin_pair,
...
def start_with_poll():
    ...
    while(True):
        ...
        for (key,val) in top_symbols.items():
            ...
            tv_url = "https://cn.tradingview.com/chart/oLP03YDC/"
            # send_email(cur_time + " 今日交易对" + coin_pair+ " 24H 涨幅:"+top_symbols[key]+"%\n"+tv_url)
            log(cur_time + " 今日交易对" + coin_pair+ " 24H 涨幅:"+top_symbols[key]+"%\n"+tv_url)
            # update_bot_pairs(coin_pair,"5150456")
            start_new_deal(coin_pair)

            i = i + 1
            if i>=2: break
        time.sleep(POLL_INTERVAL_IN_SEC1)
